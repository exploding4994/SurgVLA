...
class NeedlePick(PsmEnv):
    ...
    def _env_setup(self):
        ...
        # Disable physical interaction approximations for evaluation
        ...
        # Set correct color for the target spot with soft specular
        ...
        # Disable all collisions between PSM and the kidney to avoid physics instability
        psm_id = self.psm1.body
        keydney_id = obj_id2

        # Disable base link collisions
        p.setCollisionFilterPair(psm_id, keydney_id, -1, -1, enableCollision=0)

        # Disable collisions between each PSM active link and the kidney base
        for i in range(p.getNumJoints(psm_id)):
            p.setCollisionFilterPair(psm_id, keydney_id, i, -1, enableCollision=0)

        # Set kidney color and specular properties
    ...
    def convert_7d_to_5d_standalone(self, openvla_action, scaling=1.0):
        """ Standalone action conversion function """
