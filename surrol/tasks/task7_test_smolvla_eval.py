...
class GauzeRetrieve(PsmEnv):
    ...
    def _env_setup(self):
        ...
        # Load Spleen model
        obj_id2 = p.loadURDF(
            os.path.join(ASSET_DIR_PATH, 'Spleen/Spleen.urdf'),
        ...
        p.changeVisualShape(obj_id2, -1, specularColor=(0.3, 0.3, 0.3))

        # Disable collisions between PSM and Spleen
        psm_body_id = self.psm1.body
        spleen_id = obj_id2
        num_psm_joints = p.getNumJoints(psm_body_id)
        for link_idx in range(-1, num_psm_joints):
        ...
        self.obj_ids['fixed'].append(obj_id)
        self.obj_id, self.obj_link1 = self.obj_ids['fixed'][1], -1
...
class CustomGauzePick(GauzeRetrieve):
    ...
    def step(self, action):
        ...
        # Grasping constraints evaluation
        ...
        # Synchronize gauze position with the end-effector if grasped
        ...
        eval_result = eval_success('pick up the gauze and place it on the red target point on the spleen', self.obj_id,
                                   self.obj_id2, state[0:3])
...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        while not done and steps < max_steps:
            ...
            instruction = "pick up the gauze and place it on the red target point on the spleen"
