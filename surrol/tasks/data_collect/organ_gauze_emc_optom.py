...
class GauzeRetrieve(PsmEnv):
    ...
    def _env_setup(self):
        ...
        rd = -abs((np.random.rand() - 0.5) * 0.1 * 2)
        rd2 = (np.random.rand() - 0.5) * 0.1 * 3
        ...
        #     (workspace_limits[0].mean() + 0.5 + rd-0.05,
        ...
        # # 可选：保留或调整 specular（高光）
        # p.changeVisualShape(obj_id2, -1, specularColor=(0.25, 0.25, 0.25))  # 更柔和的高光

        # 脾脏
        # yaw = (np.random.rand() - 0.5) * np.pi

        obj_id2 = p.loadURDF(
            os.path.join(ASSET_DIR_PATH, 'Spleen/脾脏.urdf'),
            (workspace_limits[0].mean() -0.55+rd,
             workspace_limits[1].mean()+rd2,
             workspace_limits[2][0] - 0.2),
            p.getQuaternionFromEuler((0,-np.pi/2,0,)),
            useFixedBase=True,
            globalScaling=2.5
        )
        # 正确设置胰腺颜色
        p.changeVisualShape(obj_id2, -1, rgbaColor=(0.92, 0.75, 0.65, 1.0))
        # 可选：保留或调整 specular（高光）
        p.changeVisualShape(obj_id2, -1, specularColor=(0.3, 0.3, 0.3))  # 更柔和的高光

        # === 新增：禁用 PSM 与脾脏之间的碰撞 ===
        psm_body_id = self.psm1.body  # PSM 的 body ID
        spleen_id = obj_id2  # 脾脏的 body ID

        # 禁用 PSM 所有 link（包括 base -1）与脾脏（base link -1）之间的碰撞
        num_psm_joints = p.getNumJoints(psm_body_id)
        for link_idx in range(-1, num_psm_joints):  # -1 是 base link
            p.setCollisionFilterPair(
                bodyUniqueIdA=psm_body_id,
                bodyUniqueIdB=spleen_id,
                linkIndexA=link_idx,
                linkIndexB=-1,  # 脾脏通常只有一个刚体（base）
                enableCollision=0  # 0 = 关闭碰撞
            )
        # ===================================
        ...
        # gauze
        # ===== 加载纱布（gauze）=====
        gauze_x = workspace_limits[0].mean() + abs(1* (np.random.rand() - 1) * 0.1) + 0.1
        ...
        obj_id = p.loadURDF(
            ...
            (workspace_limits[0].mean() + 0.01+rd,
             workspace_limits[1].mean() +rd2,
             workspace_limits[2][0] + 0.12),
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints = [None, None, None, None,None, None, None, None]  # four waypoints
        ...
        self._waypoints[3] = np.array([pos_obj2[0], pos_obj2[1],
                                       pos_obj2[2] + (-0.0007 + 0.030) * self.SCALING, 0, -0.5])  # lift up  0.0102
...
if __name__ == "__main__":
    ...
    for idx in range(1, 101):
        ...
        #创建保存数据的文件夹
        os.makedirs('rgb/Benchmark-raw-dataset/task7_gauze_organ/rgb_images_long_224/' + str(idx),
                    exist_ok=True)
        os.makedirs('rgb/Benchmark-raw-dataset/task7_gauze_organ/depth_images_long_224/' + str(idx),
                    exist_ok=True)
        os.makedirs('rgb/Benchmark-raw-dataset/task7_gauze_organ/depth_images_long_224_32float/' + str(idx),
                    exist_ok=True)
        os.makedirs('rgb/Benchmark-raw-dataset/task7_gauze_organ/mask_images_long_224/' + str(idx),
                    exist_ok=True)
        os.makedirs('rgb/Benchmark-raw-dataset/task7_gauze_organ/state_data_long_224/' + str(idx),
                    exist_ok=True)
        ...
        while not env.are_all_actions_completed():
            ...
            # 保存各种数据...
            rgb_filename = f'rgb/Benchmark-raw-dataset/task7_gauze_organ/rgb_images_long_224/' + str(
                idx) + f'/{frame_count:06d}.png'
            cv2.imwrite(rgb_filename, rgb_img)

            depth_filename = f'rgb/Benchmark-raw-dataset/task7_gauze_organ/depth_images_long_224/' + str(
                idx) + f'/{frame_count:06d}.png'
            cv2.imwrite(depth_filename, depth_img)

            mask_filename = f'rgb/Benchmark-raw-dataset/task7_gauze_organ/mask_images_long_224/' + str(
                idx) + f'/{frame_count:06d}.png'
            cv2.imwrite(mask_filename, mask_img)

            state_filename = f'rgb/Benchmark-raw-dataset/task7_gauze_organ/state_data_long_224/' + str(
                idx) + f'/{frame_count:06d}.npy'
            np.save(state_filename, state)
            depth_filename32 = f'rgb/Benchmark-raw-dataset/task7_gauze_organ/depth_images_long_224_32float/{idx}/{frame_count:06d}.npy'
