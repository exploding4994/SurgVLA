...
class GauzeRetrieve(PsmEnv):
    ...
    def get_ecm_image(self, image_width=640, image_height=480):
        ...
        _, _, rgb_image, depth_image, mask = p.getCameraImage(
            ...
            renderer=p.ER_BULLET_HARDWARE_OPENGL  # Use higher quality renderer
        )
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints[3] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102 + 0.03) * self.SCALING, 0., -0.5])  # Lift
    ...
    def is_action_completed(self, obs, action, pos_tolerance=0.005, yaw_tolerance=0.1, gripper_tolerance=0.2):
        """
        Checks if the current action/waypoint is completed.
        """
    ...
    def are_all_actions_completed(self):
        """ Checks if all waypoints are completed. """
    ...
    def convert_7d_to_5d_standalone(self, openvla_action, scaling=1.0):
        """ Standalone version of the action conversion function """
        dx, dy, dz, da, db, dc, grip = openvla_action

        # Position processing (considering workspace and scaling)
...
class CustomGauzePick(GauzeRetrieve):
    # Success condition parameters
    Gauze_PICKUP_HEIGHT_THRESHOLD = 0.01  # Meters
    ...
    def reset(self):
        ...
        # Record original mass to restore upon release
        dyn_info = p.getDynamicsInfo(self._gauze_id, -1)
        self._gauze_original_mass = dyn_info[0]
        self._gauze_grasped = False

        # Record initial height to determine if lifted
        pos, _ = p.getBasePositionAndOrientation(self._gauze_id)
        self.gauze_initial_z = pos[2]

        # CRITICAL: Disable physical collision between the left/right grippers (links 6 and 7) and the gauze
    ...
    def step(self, action):
        ...
        # ===== Execute parent step (controls robot movement) =====
        obs, reward, done, info = super().step(action)

        # ===== Ideal grasp logic (split axis evaluation + prevent Z-axis sinking) =====
        ...
        # Grasp trigger condition
        if is_gripper_closed and (dz < Z_THRESHOLD) and (dxy < XY_THRESHOLD):
            # First time grasp condition is met: record the original Z height
        ...
        # If grasped, control gauze position (ensuring Z doesn't drop below grasp height)
        if self._gauze_grasped:
            offset_xy = np.array([0.0, 0.0])
            offset_z = -0.005  # Ideal offset slightly below gripper

            target_x = ee_pos[0]
            target_y = ee_pos[1]
            ideal_z = ee_pos[2] + offset_z

            # Ensure Z is not lower than the original recorded height
            ...
            p.resetBasePositionAndOrientation(
                self._gauze_id,
                target_pos,
                [0, 0, 0, 1]  # Keep rotation fixed
            )

        # ===== Success evaluation: grasped + lifted =====
...
if __name__ == "__main__":
    ...
    model_path = "path/to/your/openvla/model"
    ...
    for episode in range(num_episodes):
        ...
        while not done and steps < max_steps:
            ...
            # Expand dimensions to 3D for concatenation
            if gray.ndim == 2:
            ...
            # Predict action (7-DoF) using specific dataset for unnormalization
            ...
            print(f"Step {steps}: "
                  f"Grasped={info['gauze_grasped']}, "
                  f"Lifted={info['lifted']}, "
                  f"Height={info['gauze_height']:.4f}, "
                  f"Jaw={info['jaw_angle']:.4f}")
