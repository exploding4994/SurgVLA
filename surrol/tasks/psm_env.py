    ...
    def _set_action(self, action: np.ndarray):
        ...
        *2 for PSM1 [0: 5] and PSM2 [5: 10]
        ...
        for i, psm in enumerate((self.psm1, self.psm2)):
            ...
            # #========== 夹爪控制：改为增量控制 ==========
            # if self.block_gripper:
            #     # 如果block_gripper为True，强制保持闭合
            #     action[idx + 4] = -1
            #
            # # 获取当前夹爪角度（弧度）
            # current_jaw_angle = psm.get_current_jaw_position()
            #
            # # 计算目标角度：当前角度 + 动作值（动作值现在直接是弧度变化量）
            # target_jaw_angle = current_jaw_angle + action[idx + 4]
            #
            # # 限制角度范围 [0°, 80°]
            # target_jaw_angle = np.clip(target_jaw_angle,
            #                            np.deg2rad(0),  # 最小：完全闭合
            #                            np.deg2rad(80))  # 最大：完全张开
            #
            # # 执行夹爪动作
            # psm.move_jaw(target_jaw_angle)
            #
            # # 更新抓取状态
            # # 角度小于15度时认为在抓取状态（根据你的实际数据调整）
            # if target_jaw_angle < np.deg2rad(20):
            #     self._activate(i)  # 激活抓取状态
            # else:
            #     self._release(i)  # 释放状态
