...
def example_dataset_standardization(batch):
    # 检查是否存在嵌套的observation
    if "observation" in batch:
        ...
        if "image" in obs:
            ...
            if images.dtype == tf.string:
                images = tf.map_fn(
                    lambda img: tf.io.decode_image(img, channels=3, expand_animations=False),
                    images,
        ...
        if "wrist_image" in obs:
            ...
            if wrist_images.dtype == tf.string:
                wrist_images = tf.map_fn(
                    lambda img: tf.io.decode_image(img, channels=3, expand_animations=False),
                    wrist_images,
                    fn_output_signature=tf.uint8
                )
            wrist_images = tf.image.convert_image_dtype(wrist_images, tf.float32)
            wrist_images = tf.image.resize(wrist_images, (224, 224), method="bilinear")
            batch["wrist_image"] = wrist_images
...
# 注册到全局字典
OXE_STANDARDIZATION_TRANSFORMS["example_dataset"] = example_dataset_standardization